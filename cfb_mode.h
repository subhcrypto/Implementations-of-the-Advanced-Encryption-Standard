...
// CFB (Cipher Feedback) mode encryption function
void CFB_Encrypt()
{
    FILE *fp1, *fp2;
    ...
    // Initialization Vector (IV) for CFB mode
    uint8_t iv[16] = {
        0x1a, 0x2b, 0x3c, 0x4d, 
        0x5e, 0x6f, 0x70, 0x81, 
        0x92, 0xa3, 0xb4, 0xc5, 
...
// CFB (Cipher Feedback) mode decryption function
void CFB_Decrypt()
{
    FILE *fp1, *fp2;
    ...
    while(1)
    {
        ...
        // Update the IV with the current ciphertext block for the next round
        for(int i = 0; i < 16; i++)
        {
            iv[i] = buff[i];
        }
